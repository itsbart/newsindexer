...
public class StemmerFilter extends TokenFilter{
	...
	public boolean increment() throws TokenizerException {
		...
		if(_input.hasNext()){
			...
			//before stemming
			if(!wordBuff.isEmpty() && wordBuff.length() > 4 && 
    ...
    */
		...
		if(wordBuff != null && !wordBuff.isEmpty()){
			...
			if(wordBuff.endsWith("s")){
				...
				//IES -> I
					...
					wordBuff = wordBuff.substring(0, wordBuff.length() - 2);
				//S -> ""
				else if(wordBuff.charAt(wordBuff.length() - 2) != 's')
					wordBuff = wordBuff.substring(0, wordBuff.length() - 1);
			
			//words ending with *eed
			}else if(wordBuff.endsWith("eed")){
				//EED -> EE agreed -> agree
				if(wordBuff.length() > 0) wordBuff = wordBuff.substring(0, wordBuff.length() - 2);
			...
			//words ending with *ing
				...
				wordBuff = wordBuff.substring(0, wordBuff.length() - 3);
				//AT -> ATE mating -> mate
	...
	public void step2(){
		...
		if(wordBuff != null && !wordBuff.isEmpty()){
			...
			if(wordBuff.endsWith("y")){
				wordBuff = wordBuff.substring(0, wordBuff.length() - 1).concat("i");
			}
		}
	}
	...
	public void step3(){
		...
		if(wordBuff != null && !wordBuff.isEmpty() && wordBuff.length() > 3){
			...
			char var = wordBuff.charAt(wordBuff.length() - 2); //second char from last
			
			switch(var){
			
				case 'a' :
					if(wordBuff.endsWith("ational") && wordBuff.length() > 7) 
				...
				case 'o' :
					if(wordBuff.endsWith("ization")) 
						wordBuff = wordBuff.substring(0, wordBuff.length() - 5).concat("e");
					if(wordBuff.endsWith("ation"))
						wordBuff = wordBuff.substring(0, wordBuff.length() - 3).concat("e");
				...
				case 's' :
					...
					if(wordBuff.endsWith("iveness")) 
						wordBuff = wordBuff.substring(0, wordBuff.length() - 4);
			...
			switch(var){
				...
				case 'e' :
					if(wordBuff.endsWith("icate")) 
						wordBuff = wordBuff.substring(0, wordBuff.length() - 3);
					if(wordBuff.endsWith("ative") && wordBuff.length() > 7) 
						wordBuff = wordBuff.substring(0, wordBuff.length() - 5);
				...
				case 'l' :
					if(wordBuff.endsWith("ical")) 
						wordBuff = wordBuff.substring(0, wordBuff.length() - 2);
					if(wordBuff.endsWith("ful") && wordBuff.length() > 5) 
						wordBuff = wordBuff.substring(0, wordBuff.length() - 3);
				break;
				
				case 's' :
					if(wordBuff.endsWith("ness") && wordBuff.length() > 6) 
						wordBuff = wordBuff.substring(0, wordBuff.length() - 4);
					break;
	...
	public void step5(){
		...
		if(wordBuff != null && !wordBuff.isEmpty() && wordBuff.length() > 3){
			...
			switch(var){
				case 'a':
					if(wordBuff.endsWith("al") && wordBuff.length() > 5) 
						wordBuff = wordBuff.substring(0, wordBuff.length() - 2);
				break;
				
				case 'c':
					if(wordBuff.endsWith("ance") && wordBuff.length() > 7) 
						wordBuff = wordBuff.substring(0, wordBuff.length() - 4);
					if(wordBuff.endsWith("ence") && wordBuff.length() > 7) 
						wordBuff = wordBuff.substring(0, wordBuff.length() - 4);
				break;
				...
				case 'i':
					if(wordBuff.endsWith("ic") && wordBuff.length() > 4) 
						wordBuff = wordBuff.substring(0, wordBuff.length() - 2);
					break;
					
				case 'l':
					if(wordBuff.endsWith("able") && wordBuff.length() > 6) 
						wordBuff = wordBuff.substring(0, wordBuff.length() - 4);
					if(wordBuff.endsWith("ible") && wordBuff.length() > 6) 
						wordBuff = wordBuff.substring(0, wordBuff.length() - 4);
				break;
					
				case 'n':
					if(wordBuff.endsWith("ant") && wordBuff.length() > 6) 
						wordBuff = wordBuff.substring(0, wordBuff.length() - 3);
					if(wordBuff.endsWith("ement") && wordBuff.length() > 7) 
						wordBuff = wordBuff.substring(0, wordBuff.length() - 5);
					if(wordBuff.endsWith("ment") && wordBuff.length() > 6) 
						wordBuff = wordBuff.substring(0, wordBuff.length() - 4);
					if(wordBuff.endsWith("ent") && wordBuff.length() > 5) 
						wordBuff = wordBuff.substring(0, wordBuff.length() - 3);
				break;
				
				case 'o':
					if(wordBuff.endsWith("ion") && wordBuff.length() > 5) 
						wordBuff = wordBuff.substring(0, wordBuff.length() - 3);
				break;
				...
				*/
				...
				case 't':
					if(wordBuff.endsWith("ate") && wordBuff.length() > 6) 
						wordBuff = wordBuff.substring(0, wordBuff.length() - 3);
				break;
					
				case 'u' :
					if(wordBuff.endsWith("ous") && wordBuff.length() > 6) 
						wordBuff = wordBuff.substring(0, wordBuff.length() - 3);
				break;
				
				case 'v' :
					if(wordBuff.endsWith("ive") && wordBuff.length() > 6) 
						wordBuff = wordBuff.substring(0, wordBuff.length() - 3);
				break;
				
				case 'z' :
					if(wordBuff.endsWith("ize") && wordBuff.length() > 7) 
						wordBuff = wordBuff.substring(0, wordBuff.length() - 3);
				break;
